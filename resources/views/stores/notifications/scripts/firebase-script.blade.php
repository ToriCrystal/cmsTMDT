    ...
    $(document).ready(function () {
        ...
        function updateDeviceToken(deviceToken) {
            ...
            $.ajax({
                ...
                data: {store_id: userId, device_token: deviceToken},
        ...
        function getUserId() {
            let userId = null;
            @if(auth('store')->check())
                userId = @json(auth('store')->user()->id);
            @endif
                return userId;
        }
        ...
        function reloadData() {
            const userId = getUserId();
            $.ajax({
                url: urlHome + '/admin/notifications/not-read-store?store_id=' + userId,
                type: 'GET',
        ...
        function updateNotificationStatus(userId) {
            $.ajax({
                url: urlHome + '/admin/notifications/status-store',
                type: 'PATCH',
                data: {
                    store_id: userId
                },
